		...
		if(player->getTile()->hasFlag(TILESTATE_HOUSE))
		{
			...
			if(houseTile)
			{
                ...
                if(param == "none")
                {
                    ...
                    houseTile->getHouse()->setHouseOwnerAccount(0);
                }
...
bool Commands::buyHouse(Creature* creature, const std::string& cmd, const std::string& param)
{
	...
	if(player)
	{
		...
		{
			if(it->second->getHouseOwnerAccount() == player->getAccount())
			{
						...
						if(!house->getHouseOwner())
						{
							if(player->isPremium())
							{
								...
								if(price)
								{
									...
									{
                                        house->setPaidUntil(time(NULL) + (7 * 24 * 60 * 60));
										house->setHouseOwner(player->guid);
										house->setHouseOwnerAccount(player->getAccount());
