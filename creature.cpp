...
uint64_t Creature::getGainedExperience(Creature* attacker) const
{
	...
	if(player){
	   ...
	   if(randomExp <= x){
          ...
          return attacker->getPlayer() ? ((uint64_t)std::floor(getDamageRatio(attacker) * lostExperience * g_game.getExperienceStage(attacker->getPlayer()->getLevel()))) : ((uint64_t)std::floor(getDamageRatio(attacker) * lostExperience * g_config.getNumber(ConfigManager::RATE_EXPERIENCE) * 2));
       }
	   else if(randomExp <= x+y){
          g_game.addAnimatedText(attacker->getPosition(), 966, "TripleExp"); // 966, orange
	      return attacker->getPlayer() ? ((uint64_t)std::floor(getDamageRatio(attacker) * lostExperience * g_game.getExperienceStage(attacker->getPlayer()->getLevel()))) : ((uint64_t)std::floor(getDamageRatio(attacker) * lostExperience * g_config.getNumber(ConfigManager::RATE_EXPERIENCE) * 3));
	   }
