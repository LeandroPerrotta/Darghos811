...
#ifdef __USE_MYSQL__
void DBResult::addRow(MYSQL_ROW r, unsigned long* lengths, uint32_t num_fields)
{
...
#ifdef __USE_SQLITE__
void DBResult::addRow(char **results, uint32_t num_fields)
{
	RowData* rd = new RowData;
	rd->row = new char*[num_fields];
	rd->length = new unsigned long*[num_fields];
	...
	for(uint32_t i=0; i < num_fields; ++i)
	{
		...
		uint64_t colLen = strlen(results[i]);
		rd->row[i] = new char[colLen+1];
		rd->length[i] = new unsigned long;
...
int32_t DBResult::getDataInt(const std::string &s, uint32_t nrow)
{
...
int64_t DBResult::getDataLong(const std::string &s, uint32_t nrow)
{
	listNames_type::iterator it=m_listNames.find(s);
	if(it != m_listNames.end())
	{
...
std::string DBResult::getDataString(const std::string &s, uint32_t nrow)
{
	listNames_type::iterator it=m_listNames.find(s);
	if(it != m_listNames.end())
	{
...
const char* DBResult::getDataBlob(const std::string &s, unsigned long& size, uint32_t nrow)
{
	listNames_type::iterator it=m_listNames.find(s);
	if(it != m_listNames.end())
	{
...
std::string _Database::escapeString(const char* s, uint64_t size)
{
	if(s == NULL)
		return std::string("");
