...
class House
{
	public:
		...
		void setHouseOwnerAccount(uint32_t acc){houseOwnerAccount = acc;}
		uint32_t getHouseOwnerAccount(void) const {return houseOwnerAccount;}
	...
	private:
		...
		void transferToDepot(Player* player);

		bool isLoaded;
		uint32_t houseid;
		uint32_t houseOwner;
		uint32_t houseOwnerAccount;
