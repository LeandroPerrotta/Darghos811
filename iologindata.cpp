...
bool IOLoginData::saveItems(const Player* player, const ItemBlockList& itemList, DBSplitInsert& query_insert)
{
	...
	for(ItemBlockList::const_iterator it = itemList.begin(); it != itemList.end(); ++it)
	{
		...
		char buffer[attributesSize + 75];
		sprintf(buffer, "(%d, %d, %d, %d, %d, %s)", player->getGUID(), pid, runningId, item->getID(), (int32_t)item->getItemCountOrSubtype(), db->escapeBlob(attributes, attributesSize).c_str());
	...
	while(stack.size() > 0)
	{
		...
		for(uint32_t i = 0; i < container->size(); ++i)
		{
			...
			char buffer[attributesSize + 75];
			sprintf(buffer, "(%d, %d, %d, %d, %d, %s)", player->getGUID(), parentId, runningId, item->getID(), (int32_t)item->getItemCountOrSubtype(), db->escapeBlob(attributes, attributesSize).c_str());
			if(!query_insert.addRow(buffer))
				return false;
		}
	}
...
bool IOLoginData::savePlayer(Player* player, bool preSave)
{
	...
	{
	    #ifdef __CODE__
		query << "SELECT `player_id` FROM player_skills WHERE player_id = " << player->getGUID() << " AND skillid = " << i << "";
		db->storeQuery(query, result);
		...
		{
		    query.str("");
		    query << "INSERT INTO `player_skills` (`player_id`,`value`,`count`,`skillid`) values (" << player->getGUID() << "," << player->skills[i][SKILL_LEVEL] << "," << player->skills[i][SKILL_TRIES] << "," << i << ")";
		}
