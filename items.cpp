...
ItemType::ItemType()
{
	...
	isBedItem = false;
	bedPartnerDir = NORTH;
	transformToOnUse[PLAYERSEX_MALE] = 100;
	transformToOnUse[PLAYERSEX_FEMALE] = 100;
	transformToFree = 100;
}
				...
				if(readXMLInteger(itemNode, "id", intValue)){
					...
					while(itemAttributesNode)
					{
						if(readXMLString(itemAttributesNode, "key", strValue))
						{
							...
							else if(strcasecmp(strValue.c_str(), "maleTransformTo") == 0)
							{
								if(readXMLInteger(itemAttributesNode, "value", intValue))
									it.transformToOnUse[PLAYERSEX_MALE] = intValue;
							}
							else if(strcasecmp(strValue.c_str(), "femaleTransformTo") == 0)
							{
								if(readXMLInteger(itemAttributesNode, "value", intValue))
					...
					if(it.transformToOnUse[PLAYERSEX_MALE] == 100 && it.transformToOnUse[PLAYERSEX_FEMALE] != 100)
						std::cout << "Warning: [Items::loadFromXml] maleTransformTo missing for ItemID " << id << std::endl;
					else if(it.transformToOnUse[PLAYERSEX_MALE] != 100 && it.transformToOnUse[PLAYERSEX_FEMALE] == 100)
						std::cout << "Warning: [Items::loadFromXml] femaleTransformTo missing for ItemID " << id << std::endl;
