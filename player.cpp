...
void Player::onCreatureDisappear(const Creature* creature, uint32_t stackpos, bool isLogout)
{
	...
	if(creature == this)
	{
		...
		{
			std::vector<boost::shared_ptr<RuleViolation> > stack;
			for(RuleViolationsMap::const_iterator rt = g_game.getRuleViolations().begin(); rt != g_game.getRuleViolations().end(); ++rt)
			{
				boost::shared_ptr<RuleViolation> rvr = rt->second;
				if(rvr->gamemaster == this)
					stack.push_back(rvr);
			}

			for(std::vector<boost::shared_ptr<RuleViolation> >::iterator rt = stack.begin(); rt != stack.end(); ++rt)
				g_game.playerCloseRuleViolation((*rt)->gamemaster->getID(), (*rt)->reporter->getName());
		}
