...
bool ProtocolGame::login(const std::string& name, uint32_t accnumber, const std::string& password, uint16_t operatingSystem, uint8_t gamemasterLogin)
{
	...
	if(!_player || g_config.getNumber(ConfigManager::ALLOW_CLONES) || name == "Account Manager")
	{
		...
		if(g_bans.isAccountBanished(accnumber) && !player->hasFlag(PlayerFlag_CannotBeBanned) && g_bans.getBanTime(accnumber) > timeNow)
		{
			std::string name_;
			if(g_bans.getBannedBy(accnumber) == 0)
				name_ = "Automatic banishment";
			else
				IOLoginData::getInstance()->getNameByGuid(g_bans.getBannedBy(accnumber), name_);
			char date[16];
		...
		if(g_bans.isAccountDeleted(accnumber) && g_bans.getBanTime(accnumber) != 0 && g_bans.getBanTime(accnumber) <= timeNow)
		{
			std::string name_;
			if(g_bans.getBannedBy(accnumber) == 0)
				name_ = "Automatic deletion";
			else
				IOLoginData::getInstance()->getNameByGuid(g_bans.getBannedBy(accnumber), name_);
			char date[16];
			formatDate2(g_bans.getBanTime(accnumber), date);
...
bool ProtocolGame::logout(bool displayEffect, bool forced)
{
	...
	if(!player->isRemoved())
	{
   		if(!forced)
   		{
   			if(player->getTile()->hasFlag(TILESTATE_NOLOGOUT) && player->getAccountType() < ACCOUNT_TYPE_GOD)
   			{
   				player->sendCancelMessage(RET_YOUCANNOTLOGOUTHERE);
   				return false;
   			}

   			if(player->hasCondition(CONDITION_INFIGHT) && player->getAccountType() < ACCOUNT_TYPE_GOD)
   			{
   			...
   			if(!g_creatureEvents->playerLogout(player) && player->getAccountType() < ACCOUNT_TYPE_GOD)
   			{
...
bool ProtocolGame::parseFirstPacket(NetworkMessage& msg)
{
	...
	if(version != 811)
	{
		disconnectClient(0x0A, "Only clients with protocol 8.11 allowed!");
		return false;
	}
...
/*                if(m_pingUpdate == 0 || OTSYS_TIME() >= m_pingUpdate + (30 * 1000))
                {
                    if (m_pingValue != 0)
                    {
                        DBQuery query;
                        ...
                        m_pingFinal = (m_pingTest1 + m_pingTest2 + m_pingTest3) / 3;

                        db = Database::getInstance();
                        query << "UPDATE `players` SET `ping` = " << m_pingFinal << " WHERE `name` = '" << player->getName() << "'";

                        if (!db->executeQuery(query))
                        {
                            std::cout << "WARNING [Protocolgame::updatePing]: Não foi possivel salvar o ping do jogador " << player->getName() << ".";
                        }
                    ...
                    m_pingUpdate = OTSYS_TIME();
                }

                if(m_pingTest1 == 0)
                    m_pingTest1 = OTSYS_TIME() - m_pingValue;
                else if(m_pingTest2 == 0)
                    m_pingTest2 = OTSYS_TIME() - m_pingValue;
                else if(m_pingTest3 == 0)
                    m_pingTest3 = OTSYS_TIME() - m_pingValue;*/
			...
			default:
				std::cout << "Unknown byte received: 0x" << std::hex << (int16_t)recvbyte << ", from player: " << player->getName() << "." << std::endl;
				break;
		}
	}
}
...
void ProtocolGame::checkCreatureAsKnown(uint32_t id, bool &known, uint32_t &removedKnown)
{
	...
	if(knownCreatureList.size() > 150) //150 for 8.1
	{
...
void ProtocolGame::AddCreature(NetworkMessage* msg, const Creature* creature, bool known, uint32_t remove)
{
	...
	if(targetPlayer && player->getAccountType() < ACCOUNT_TYPE_GAMEMASTER && targetPlayer->isInGhostMode())
		msg->AddByte(0x00);
	else
