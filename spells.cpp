...
bool ConjureSpell::ConjureItem(const ConjureSpell* spell, Creature* creature, const std::string& param)
{
	...
	if(spell->getReagentId() != 0)
	{
		...
		Item* item = player->getInventoryItem(SLOT_LEFT);
		if(item && item->getID() == spell->getReagentId())
		{
			if(internalConjureItem(player, spell->getConjureId(), spell->getConjureCount(),
				spell->reagentTransform(), spell->getReagentId(), SLOT_LEFT))
			{
		...
		{
			if(internalConjureItem(player, spell->getConjureId(), spell->getConjureCount(),
				spell->reagentTransform(), spell->getReagentId(), SLOT_RIGHT))
			{
				spell->postCastSpell(player, true);
				result = true;
			}
