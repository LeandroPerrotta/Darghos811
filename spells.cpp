...
bool ConjureSpell::internalConjureItem(Player* player, uint32_t conjureId, uint32_t conjureCount,
	bool transform /*= false*/, uint32_t reagentId /*= 0*/, slots_t slot /*= SLOT_WHEREVER*/)
{
	if(!transform)
	{
		...
		if(reagentId != 0 && ret == RET_NOERROR)
			g_game.internalRemoveItem(player->getInventoryItem(slot), conjureCount);
...
bool ConjureSpell::ConjureItem(const ConjureSpell* spell, Creature* creature, const std::string& param)
{
	...
	if(spell->getReagentId() != 0)
	{
		...
		Item* itemLeft = player->getInventoryItem(SLOT_LEFT);
		Item* itemRight = player->getInventoryItem(SLOT_RIGHT);
		if(itemLeft && itemLeft->getID() == spell->getReagentId())
		{
			slot = SLOT_LEFT;
			found = true;
		}
		else if(itemRight && itemRight->getID() == spell->getReagentId())
		{
			slot = SLOT_RIGHT;
			found = true;
		}
		...
		if(found)
		{
			...
			if(internalConjureItem(player, spell->getConjureId(), spell->getConjureCount(),
				spell->reagentTransform(), spell->getReagentId(), slot))
				result = true;
