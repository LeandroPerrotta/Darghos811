...
player->setNextActionTask(NULL);
	...
	if(isHotkey)
	{
		...
		std::string currentName = item->getName();
...
void Actions::showUseHotkeyMessage(Player* player, const std::string& name, uint32_t itemCount, const std::string& plural)
{
	char buffer[100];
	if(itemCount == 1)
		sprintf(buffer, "Using the last %s...", name.c_str());
	else
		sprintf(buffer, "Using one of %d %s...", itemCount, plural.c_str());
...
{
	if(strcasecmp(functionName.c_str(), "increaseItemId") == 0)
