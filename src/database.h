...
class DBQuery : public std::stringstream
{
	public:
		...
		*/
		int32_t getSize(){return (int)this->str().length();}
...
class DBResult
{
	public:
		...
		*/
		int32_t getDataInt(const std::string &s, uint32_t nrow = 0);
		...
		*\param nrow The number of the row
		*/
		int64_t getDataLong(const std::string &s, uint32_t nrow = 0);
		...
		*\param nrow The number of the row
		*/
		std::string getDataString(const std::string &s, uint32_t nrow=0);
		...
		*\param nrow The number of the row
		*/
		const char* getDataBlob(const std::string &s, unsigned long& size, uint32_t nrow=0);
	
		/** Get the number of rows
		*\returns The number of rows
		*/
		uint32_t getNumRows() {return m_numRows;}
	
		/** Get the number of fields
		*\returns The number of fields
		*/
		uint32_t getNumFields() {return m_numFields;}
	
	private:
		//friend class Database;
		#ifdef __USE_MYSQL__
		friend class DatabaseMySQL;
		void addRow(MYSQL_ROW r, unsigned long* lengths, uint32_t num_fields);
		#endif
		#ifdef __USE_SQLITE__
		friend class DatabaseSqLite;
		void addRow(char **results, uint32_t num_fields);
		#endif
		void clear();
		void setFieldName(const std::string &s, uint32_t n)
		{
			m_listNames[s] = n;
			m_numFields++;
		};
		...
		uint32_t m_numRows;
		typedef std::map<const std::string, uint32_t> listNames_type;
		listNames_type m_listNames;
		typedef std::map<uint32_t, RowData* > RowDataMap;
...
class _Database
{
	public:
		...
		*\param s The source string
		*/
		static std::string escapeString(const char* s, uint64_t size);
