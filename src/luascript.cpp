...
void LuaScriptInterface::registerFunctions()
{
    ...
    //setHouseOwnerAccount(houseid, ownerGUID)
	lua_register(m_luaState, "setHouseOwnerAccount", LuaScriptInterface::luaSetHouseOwnerAccount);
...
int32_t LuaScriptInterface::luaSetHouseOwnerAccount(lua_State* L)
{
    //setHouseOwnerAccount(houseid, ownerAccount)
    uint32_t ownerAccount = popNumber(L);
    uint32_t houseid = popNumber(L);

    House* house = Houses::getInstance().getHouse(houseid);
    if(house)
    {
        house->setHouseOwnerAccount(ownerAccount);
...
#define MULTIOP(name, op) \
	int32_t LuaScriptInterface::luaBit##name(lua_State* L) { \
		int32_t n = lua_gettop(L), i, w = popNumber(L); \
		for(i = 2; i <= n; ++i){ \
			w op popNumber(L); \
		} \
		lua_pushnumber(L, w); \
...
#define SHIFTOP(name, op) \
	int32_t LuaScriptInterface::luaBit##name(lua_State* L) { \
		int32_t n2 = popNumber(L), n1 = popNumber(L); \
