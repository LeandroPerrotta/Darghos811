...
#endif
{
	...
	for(int16_t i = 0; i < 3; i++)
		g_game.setServerSaveMessage(i, false);
	...
	{
		#ifndef __CONSOLE__
		if(MessageBox(gui.m_mainWindow, "Unable to load items (OTB)! Continue?", "Items (OTB)", MB_YESNO) == IDNO)
		{
		#endif
	...
	{
		#ifndef __CONSOLE__
		if(MessageBox(gui.m_mainWindow, "Unable to load items (XML)! Continue?", "Items (XML)", MB_YESNO) == IDNO)
		{
		#endif
	...
	{
		#ifndef __CONSOLE__
		if(MessageBox(gui.m_mainWindow, "Unable to load monsters! Continue?", "Monsters", MB_YESNO) == IDNO)
		{
		#endif
	...
	#ifndef __CONSOLE__
	SendMessage(gui.m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Loading quests");
	#endif
	if(!Quests::getInstance()->loadFromXml())
	{
		#ifndef __CONSOLE__
		if(MessageBox(gui.m_mainWindow, "Unable to load quests! Continue?", "Quests", MB_YESNO) == IDNO)
		{
		#endif
	...
	if(strcasecmp(g_config.getString(ConfigManager::PASSWORDTYPE).c_str(), "md5") == 0)
	{
		g_config.setNumber(ConfigManager::PASSWORD_TYPE, PASSWORD_TYPE_MD5);
		std::cout << ">> Using MD5 passwords" << std::endl;
	}
	if(strcasecmp(g_config.getString(ConfigManager::PASSWORDTYPE).c_str(), "sha1") == 0)
	{
	...
	#ifndef __CONSOLE__
	SendMessage(gui.m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Loading raids");
	#endif
